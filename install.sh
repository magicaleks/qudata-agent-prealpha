...
# --- Шаг 4: GPU (универсальная проверка) ---
echo -e "${YELLOW}[5/11] Checking for GPU...${NC}"
...
if [ -n "$GPU_INFO" ]; then
    ...
    if echo "$GPU_VENDOR" | grep -iq "NVIDIA"; then
        ...
        if ! dpkg -l | grep -q nvidia-container-toolkit 2>/dev/null; then
            ...
            # Добавляем репозиторий с правильным путем для дистрибутива
            curl -s -L https://nvidia.github.io/libnvidia-container/$distribution/libnvidia-container.list | \
                sed 's#deb https://#deb [signed-by=/usr/share/keyrings/nvidia-container-toolkit-keyring.gpg] https://#g' | \
                tee /etc/apt/sources.list.d/nvidia-container-toolkit.list > /dev/null
            
            # Обновляем список пакетов и устанавливаем toolkit
            apt-get update -qq
